# 2D Patch Prediction Fusion + Soft Voting
...
def _rotate_mirror_do(im):
    """
    Duplicate an np array (image) of shape (x, y, nb_channels) 4 times, in order
...
def _rotate_mirror_undo(im_mirrs):
	...
	origs.append(np.array(im_mirrs[2])[:, ::-1])
	origs.append(np.rot90(np.array(im_mirrs[3]), axes=(0, 1), k=2)[:, ::-1])
