...
class Unet:
    ...
    def root_1(self, x):
        conv1 = self.conv_bn_relu(x, 8, 3, 3, 3)
        conv1 = self.conv_bn_relu(conv1, 8, 3, 3, 3)
        pool1 = MaxPooling3D(pool_size=(2, 2, 2))(conv1)
        
        conv2 = self.conv_bn_relu(pool1, 16, 3, 3, 3)
        conv2 = self.conv_bn_relu(conv2, 16, 3, 3, 3)
        pool2 = MaxPooling3D(pool_size=(2, 2, 2))(conv2)

        conv3 = self.conv_bn_relu(pool2, 32, 3, 3, 3)
        conv3 = self.conv_bn_relu(conv3, 32, 3, 3, 3)
        pool3 = MaxPooling3D(pool_size=(2, 2))(conv3)

        conv4 = self.conv_bn_relu(pool3, 64, 3, 3, 3)
        conv4 = self.conv_bn_relu(conv4, 64, 3, 3, 3)
        pool4 = MaxPooling3D(pool_size=(2, 2, 2))(conv4)

        conv4_1 = self.conv_bn_relu(pool4, 128, 3, 3, 3)
        conv4_1 = self.conv_bn_relu(conv4_1, 128, 3, 3, 3)
        pool4_1 = MaxPooling3D(pool_size=(2, 2, 2))(conv4_1)

        conv5 = self.conv_bn_relu(pool4_1, 256, 3, 3, 3)
        conv5 = self.conv_bn_relu(conv5, 256, 3, 3, 3)

        up5_1 = concatenate([UpSampling3D(size=(2, 2))(conv5), conv4_1], axis=-1)
        conv5_1 = self.conv_bn_relu(up5_1, 128, 3, 3, 3)
        conv5_1 = self.conv_bn_relu(conv5_1, 128, 3, 3, 3)

        up6 = concatenate([UpSampling3D(size=(2, 2))(conv5_1), conv4], axis=-1)
        conv6 = self.conv_bn_relu(up6, 64, 3, 3, 3)
        conv6 = self.conv_bn_relu(conv6, 64, 3, 3, 3)

        up7 = concatenate([UpSampling3D(size=(2, 2, 2))(conv6), conv3], axis=-1)
        conv7 = self.conv_bn_relu(up7, 32, 3, 3, 3)
        conv7 = self.conv_bn_relu(conv7, 32, 3, 3, 3)

        up8 = concatenate([UpSampling3D(size=(2, 2, 2))(conv7), conv2], axis=-1)
        conv8 = self.conv_bn_relu(up8, 16, 3, 3, 3)
        conv8 = self.conv_bn_relu(conv8, 16, 3, 3, 3)

        up9 = concatenate([UpSampling3D(size=(2, 2, 2))(conv8), conv1], axis=-1)
        conv9 = self.conv_bn_relu(up9, 8, 3, 3, 3)
        conv9 = self.conv_bn_relu(conv9, 8, 3, 3, 3)

        conv10 = Convolution3D(1, (1, 1, 1), activation='sigmoid')(conv9)
        return conv10

    def model(self, dimension_size1, dimension_size2, dimension_size3, loss="categorical_crossentropy", learning_rate=0.01):
        image = Input((dimension_size1, dimension_size2, dimension_size3, 1))
