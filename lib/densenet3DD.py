...
from keras.layers.convolutional import Conv2D, Conv2DTranspose, UpSampling2D, Conv3D, UpSampling3D, Conv3DTranspose
from keras.layers.pooling import AveragePooling2D, MaxPooling2D, AveragePooling3D, MaxPooling3D
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4):
    ...
    if bottleneck:
        ...
        x = Conv3D(inter_channel, (1, 1, 1), kernel_initializer='he_normal', padding='same', use_bias=False,
                    dilation_rate=1, kernel_regularizer=l2(weight_decay))(x)
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    #print ("4" , np.shape(x), ((np.shape(img_input)[1])/(np.shape(x)[1])))

    x = Conv3D(32, 1, kernel_initializer='he_normal', padding='same',
               strides=initial_strides, use_bias=False, kernel_regularizer=l2(weight_decay))(x)

    print ("5" , np.shape(x))

    x = Reshape((256, 256, nb_classes))(x)
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    # Initial convolution
    x = Conv3D(init_conv_filters, (2, 2, 2), activation='relu', padding='same', strides=(2, 2, 2),
    ...
    x_up = Conv3DTranspose(init_conv_filters, (2, 2, 2), activation='relu', padding='same', strides=(2, 2, 2),
